...
export function PredictivePriceAlert({ currentPrice, dataPromise }: PredictivePriceAlertProps) {
  const data = use(dataPromise);
  
  // Calculate recommended initial target (5% off)
  ...
  const [scanResult, setScanResult] = useState<{ found: boolean; hour?: number; message: string } | null>(null);

  // Find the absolute lowest price in the next 24 hours
  ...
  const handleScan = () => {
    startTransition(() => {
       // Simulate a slight network/computational delay for the "Radar" effect
       setTimeout(() => {
         const match = data.find(d => d.price <= targetPrice);
    ...
    <div className="bg-slate-900 border border-amber-500/30 rounded-2xl p-6 shadow-2xl relative overflow-hidden group">
      ...
      <div className="flex flex-col sm:flex-row items-end gap-4 relative z-0">
        <div className="w-full sm:w-1/2">
          <label htmlFor="target-price" className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">
            Target Price ($)
          </label>
        ...
        <div className="w-full sm:w-1/2 flex gap-2 sm:pb-3">
           <button onClick={() => handleQuickSet(0.05)} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 rounded-lg py-2 text-xs font-bold transition-colors">
              -5%
           </button>
           <button onClick={() => handleQuickSet(0.10)} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 rounded-lg py-2 text-xs font-bold transition-colors">
              -10%
           </button>
           <button onClick={() => handleQuickSet(0.20)} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 rounded-lg py-2 text-xs font-bold transition-colors">
              -20%
           </button>
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 relative z-0">
         ...
         <div className="flex-1 w-full sm:w-auto h-12 flex items-center justify-end">
           ...
           {!scanResult && !isPending && (
             <span className="text-xs text-slate-500 font-mono italic">Waiting for target input...</span>
           )}
         </div>
      </div>
      
      {/* Mobile Best Time Badge (moves here on small screens) */}
      {bestTime && (
