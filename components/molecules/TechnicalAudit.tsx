...
export function TechnicalAudit({ cacheStatus, latencyPromise }: TechnicalAuditProps) {
  ...
  let latencyColorClass = "text-slate-900 dark:text-slate-300 bg-slate-500/10";
  if (latency !== null) {
    if (latency < 5) latencyColorClass = "text-emerald-900 dark:text-emerald-300 bg-emerald-500/10";
    else if (latency < 15) latencyColorClass = "text-amber-900 dark:text-amber-300 bg-amber-500/10";
    else latencyColorClass = "text-rose-900 dark:text-rose-300 bg-rose-500/10";
  }
  ...
  return (
    <div className="tour-technical-audit p-5 mt-8 rounded-2xl bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 font-mono text-sm space-y-3 text-slate-600 dark:text-slate-400">
      <h2 className="text-slate-900 dark:text-slate-100 font-bold uppercase tracking-widest mb-4 text-xs flex items-center gap-2">
        <span className="material-symbols-outlined notranslate text-emerald-500 text-[1.1rem]" aria-hidden="true" translate="no">analytics</span>
        Technical Audit
      </h2>
      <div className="flex justify-between items-center bg-white dark:bg-slate-900/50 px-4 py-3 rounded-lg shadow-sm border border-slate-200 dark:border-slate-800">
        <span className="font-semibold text-xs text-slate-700 dark:text-slate-300 uppercase">Edge Cache Status</span>
        <span className="text-emerald-900 dark:text-emerald-300 font-bold tracking-tight bg-emerald-500/10 px-2 py-0.5 rounded text-xs">{cacheStatus}</span>
      </div>
      <div className="flex justify-between items-center bg-white dark:bg-slate-900/50 px-4 py-3 rounded-lg shadow-sm border border-slate-200 dark:border-slate-800">
        <span className="font-semibold text-xs text-slate-700 dark:text-slate-300 uppercase">Wasm Execution Latency</span>
        <span 
