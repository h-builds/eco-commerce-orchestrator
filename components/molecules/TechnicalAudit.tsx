...
export function TechnicalAudit({ cacheStatus, latencyPromise }: TechnicalAuditProps) {
  ...
  return (
    <div className="p-5 mt-8 rounded-2xl bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 font-mono text-sm space-y-3 text-slate-600 dark:text-slate-400">
      ...
      <div className="flex justify-between items-center bg-white dark:bg-slate-900/50 px-4 py-3 rounded-lg shadow-sm border border-slate-200 dark:border-slate-800">
        ...
        <span className="text-amber-500 font-bold tracking-tight bg-amber-500/10 px-2 py-0.5 rounded text-xs">{latency !== null ? `${latency.toFixed(2)}ms` : '...'}</span>
      </div>
    </div>
  );
}
